document.addEventListener('DOMContentLoaded', () => {
	...
	if (newsletters) {
		// newsletters confirm close
		const newslettersConfirm = document.querySelector('.confirm-newsletters');
		if (newslettersConfirm) {
			const newslettersConfirmClose = document.querySelector('.confirm-newsletters__close');
			newslettersConfirmClose.addEventListener('click', () => {
				newslettersConfirm.classList.remove('active');
			});
		}

		const inputs = document.querySelectorAll('.newsletters__input input');
		newslettersButton = document.querySelector('.newsletters__button');

		newsletters.addEventListener('submit', (e) => {
			e.preventDefault();
			const isValid = Array.from(inputs).every(validateField);
			if (isValid) {
				newsletters.submit();
				if (newslettersConfirm) {
					newslettersConfirm.classList.add('active');
				}
			}
		});
